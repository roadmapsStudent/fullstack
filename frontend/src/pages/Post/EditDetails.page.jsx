...
function PostDetailsPage(props) {
  ...
  const SECONDARY_COL_HEIGHT = `calc(${PRIMARY_COL_HEIGHT} / 6 - ${theme.spacing.md} / 6)`;
  const SECONDARY_DOUBLE_HEIGHT = `calc(${PRIMARY_COL_HEIGHT} / 3 - ${theme.spacing.md} / 3)`;
      ...
      <Box maw={600} mx="auto">
        <form onSubmit={form.onSubmit(handleSubmit)}>
          ...
          <Group position="right" mt="md">
            <Button className={classes.Paper} onClick={() => { setEditing(0); }}>
...
export const postDetailsLoader = async ({ params }) => {
  ...
  //const res = await axios.get(`${DOMAIN}/api/posts/:id=`+params.id);
  return res.data;
};
